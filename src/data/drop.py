...
class Drop(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        dl_dirs = dl_manager.download_and_extract([_URL, 'https://github.com/inbaroren/improving-compgen-in-semparse/archive/refs/heads/main.zip'])
        data_dir = os.path.join(dl_dirs[0], "drop_dataset")
        icgs_split_dir = os.path.join(dl_dirs[1], "improving-compgen-in-semparse-main/data/DROP")
        icgs_train_split = os.path.join(icgs_split_dir, "train.json")
        icgs_test_split = os.path.join(icgs_split_dir, "heldout_test.json")
        return [
            ...
            datasets.SplitGenerator(
                name='icgs_template_train',
                # These kwargs will be passed to _generate_examples
                gen_kwargs={"filepath": data_dir, "split": icgs_train_split},
            ),
            datasets.SplitGenerator(
                name='icgs_template_test',
                # These kwargs will be passed to _generate_examples
                gen_kwargs={"filepath": data_dir, "split": icgs_test_split},
            ),
        ]

    def get_drop_instances(self, filepath, split):
            ...
            for i, (section_id, section) in enumerate(data.items()):
                for j, qa in enumerate(section["qa_pairs"]):
                    ...
                    try:
                        ...
                        all_answers_spans = [self.build_answers([answer]) for answer in answers]
                        if any([len(answer["spans"]) > 1 for answer in all_answers_spans]):
                            continue
                        if len(set([answer["spans"][0] for answer in all_answers_spans])) > 1:
                            continue
                        # example["answers_spans"] = all_answers_spans[0]
                        example["answer_text"] = all_answers_spans[0]["spans"][0]
                        example["answer_type"] = all_answers_spans[0]["types"][0]
                        if len(example["answer_text"].split()) > 5:
                            continue
                        yield id_, example
                        id_ += 1
    ...
    def _generate_examples(self, filepath, split):
        ...
        if split in [datasets.Split.TRAIN, datasets.Split.VALIDATION]:
            yield from self.get_drop_instances(filepath, split)
        else:
            trn_instances = list(self.get_drop_instances(os.path.join(filepath, "drop_dataset_train.json"), "train"))
            dev_instances = list(self.get_drop_instances(os.path.join(filepath, "drop_dataset_dev.json"), "validation"))
            import json
            # breakpoint()
            d = json.load(open(split))
            qids = set([ex['question_id'] for ex in d])
            for _, ex in trn_instances:
                if f'DROP_train_{ex["section_id"]}_{ex["query_id"]}' in qids:
                    yield ex['query_id'], ex
            for _, ex in dev_instances:
                if f'DROP_dev_{ex["section_id"]}_{ex["query_id"]}' in qids:
                    yield ex['query_id'], ex
